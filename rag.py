...
with st.sidebar:
    ...
    # Upload PDF (EDA report)
    pdf_file = st.file_uploader("Upload EDA Report (PDF)", type=["pdf"], key="pdf_uploader")
    
    # Upload CSV files (datasets)
    csv_file1 = st.file_uploader("Upload Dataset 1 (CSV)", type=["csv"], key="csv_uploader1")
    csv_file2 = st.file_uploader("Upload Dataset 2 (CSV)", type=["csv"], key="csv_uploader2")
    
    if API_KEY and (pdf_file or csv_file1 or csv_file2):
            ...
            try:
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    if pdf_file:
                        ...
                        st.sidebar.success(f"Processed PDF: {pdf_file.name}")
                    ...
                    if csv_file1:
                        try:
                            ...
                            st.sidebar.success(f"Loaded Dataset 1: {csv_file1.name} ({df1.shape[0]} rows)")
                        except Exception as e:
                    ...
                    if csv_file2:
                        try:
                            ...
                            st.sidebar.success(f"Loaded Dataset 2: {csv_file2.name} ({df2.shape[0]} rows)")
                        except Exception as e:
        ...
        if dataset_choice:
            ...
            if analysis_type == "Summary Statistics":
                ...
                amount_cols = [col for col in df.select_dtypes(include=['number']).columns 
                              if any(term in col.lower() for term in ['amount', 'price', 'cost', 'revenue', 'income', 'sale'])]
